"""Main class for generating books using AutoGen - Updated for AutoGen v2"""
...
class BookGenerator:
    def __init__(self, agents: Dict, agent_config: Dict, outline: List[Dict]):
    ...
    async def generate_chapter_async(self, chapter_number: int, prompt: str) -> None:
        """Generate a single chapter with completion verification"""
        print(f"\nGenerating Chapter {chapter_number}...")
        
        try:
            # Create team for chapter generation
            chapter_team = RoundRobinGroupChat([
            ...
            4. Writer: Final Version (SCENE FINAL)

            Wait for each step to complete before proceeding."""

            # Start generation
            result = await chapter_team.run(task=chapter_prompt)
            ...
            if not self._verify_chapter_complete(messages):
            ...
            await self._handle_chapter_generation_failure(chapter_number, prompt)

    def generate_chapter(self, chapter_number: int, prompt: str) -> None:
        """Synchronous wrapper for chapter generation"""
        asyncio.run(self.generate_chapter_async(chapter_number, prompt))
    ...
    def _verify_chapter_complete(self, messages: List[Dict]) -> bool:
        ...
        for msg in messages:
            ...
            if "SCENE:" in content and "SCENE FINAL:" not in content: sequence_complete['scene'] = True
            ...
            print("******************** CHAPTER_CONTENT ****************", bool(chapter_content))
        
        # Verify all steps completed and content exists
        essential_complete = sequence_complete['memory_update'] and sequence_complete['scene_final']
        if essential_complete and current_chapter and chapter_content:
    ...
    async def _handle_chapter_generation_failure(self, chapter_number: int, prompt: str) -> None:
        """Handle failed chapter generation with simplified retry"""
        print(f"Attempting simplified retry for Chapter {chapter_number}...")
        
        try:
            # Create a simplified team with just essential agents
            retry_team = RoundRobinGroupChat([
...
Keep it simple and direct."""
            ...
            result = await retry_team.run(task=retry_prompt)
    ...
    def _save_chapter(self, chapter_number: int, content_or_messages) -> None:
        """Save chapter content to file"""
        print(f"\nSaving Chapter {chapter_number}")
        try:
            # Handle both direct content and message lists
            if isinstance(content_or_messages, str):
                chapter_content = content_or_messages
            else:
                chapter_content = self._extract_final_scene(content_or_messages)
    ...
    async def generate_book_async(self, outline: List[Dict]) -> None:
        ...
        for chapter in sorted_outline:
            ...
            await self.generate_chapter_async(chapter_number, chapter["prompt"])
    ...
    def generate_book(self, outline: List[Dict]) -> None:
        """Synchronous wrapper for book generation"""
        asyncio.run(self.generate_book_async(outline))
