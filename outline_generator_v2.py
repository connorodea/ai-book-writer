"""Generate book outlines using AutoGen agents - Updated for AutoGen v2"""
...
class OutlineGenerator:
    def __init__(self, agents: Dict, agent_config: Dict):
        self.agents = agents
        self.agent_config = agent_config

    async def generate_outline_async(self, initial_prompt: str, num_chapters: int = 25) -> List[Dict]:
        """Generate a book outline based on initial prompt using async API"""
        print("\nGenerating outline...")

        # Create the team for outline generation
...
End the outline with 'END OF OUTLINE'"""
        ...
        try:
            # Run the team chat
            result = await outline_team.run(task=outline_prompt)
            ...
            return self._process_outline_results(messages, num_chapters)
    ...
    def generate_outline(self, initial_prompt: str, num_chapters: int = 25) -> List[Dict]:
        """Synchronous wrapper for outline generation"""
        return asyncio.run(self.generate_outline_async(initial_prompt, num_chapters))
    ...
    def _process_outline_results(self, messages: List[Dict], num_chapters: int) -> List[Dict]:
        ...
        if len(chapters) < num_chapters:
            print(f"Only processed {len(chapters)} valid chapters out of {num_chapters} required")
            # Add placeholder chapters for missing ones
            chapters = self._verify_chapter_sequence(chapters, num_chapters)
